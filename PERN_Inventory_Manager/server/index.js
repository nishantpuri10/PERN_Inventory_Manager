...
app.post("/showcart", async (req, res) => {
  ...
  const query = `
    SELECT product.name AS productname, product.company, product.price
    FROM buyer
...
app.post("/calculateTotalPrice", async (req, res) => {
  ...
  const query = `
    SELECT SUM(product.price) AS total_price
    FROM buyer
    JOIN cart ON buyer.buyerid = cart.buyer_id
    JOIN product ON cart.product_id = product.productid
    WHERE buyer.buyerid = $1
...
////Show buyer name , email and products he bought and price /////
app.post('/showBuyerAndProducts', async (req, res) => {
  try {
    const { userid } = req.body;

    const query = `
      SELECT buyer.name AS buyer_name, ARRAY_AGG(product.name) AS product_names, SUM(product.price) AS total_price
      FROM buyer
